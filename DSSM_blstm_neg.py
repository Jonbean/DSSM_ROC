...
class DSSM_BLSTM_Model(object):
    def __init__(self):
        ...
        self.best_model_save_path = './BLSTM_neg_best_val_model_params.pkl'
        self.best_test_model_save_path = './BLSTM_neg_best_test_model_params.pkl'
    ...
    def model_constructor(self, wemb_matrix_path = None):
        ...
        self.doc_encoder = BLSTM_Encoder.BlstmEncoder(300)
        self.query_encoder = BLSTM_Encoder.BlstmEncoder(300)
    ...
    def saving_model(self, val_or_test):
        ...
        if val_or_test == 'val':
            pickle.dump((doc_all_params_value, query_all_params_value), open(self.best_val_model_save_path, 'wb'))
        else:
            pickle.dump((doc_all_params_value, query_all_params_value), open(self.best_test_model_save_path, 'wb'))

    def reload_model(self, val_or_test):
        if val_or_test == 'val':    
            doc_encoder_params, query_encoder_params = pickle.load(open(self.best_val_model_save_path))
            ...
            doc_encoder_params, query_encoder_params = pickle.load(open(self.best_test_model_save_path))
            lasagne.layers.set_all_param_values(self.doc_encoder.output, doc_encoder_params)
            lasagne.layers.set_all_param_values(self.query_encoder.output, query_encoder_params)  
